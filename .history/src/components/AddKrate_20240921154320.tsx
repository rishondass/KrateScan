...
const AddKrate = ({toggle}:Props) => {
  const usersetUser] = useUser(state=>[state.user,state.setUser]);
  const [dataUri, setDataUri] = useState("")
  const [toggleCamera, setToggleCamera] = useState(false);
  const [krateInfo, setKrateInfo] = useState<krate>({name:"",description:"",location:"",image:"",id: uuid(), items: []});
  ...
  async function saveKrate(){
    ...
    setUser(temp);
    const save = await fetch('/api/v1/'+user.username,{
      method: "POST",
