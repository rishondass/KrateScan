...
const ItemModal = ({krateID, id,name, description, quantity, image, toggle,handleEdit}:Props) => {
  ...
  const [itemInfo, setItemInfo] = useState<itemType>({name:name||"",description:description||"",quantity:quantity||1,image:image||(uuid()+".png"),id: id||uuid(),krateID: krateID || ""});
  ...
  async function saveItem(){
    ...
    const save = await fetch("/api/v1/item/"+id,{
      method: "PUT",
      body: JSON.stringify({...itemInfo, newImage: newImageURL, imageUri: imageUri}),
      headers: {
        'Content-Type': 'application/json',
      }
    });
